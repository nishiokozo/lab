...
//-----------------------------------------------------------------------------
gra.pset = function( ox, oy, [r, g, b, a] )
//-----------------------------------------------------------------------------
{
	let x = Math.floor(ox);
	let y = Math.floor(oy);
...
//-----------------------------------------------------------------------------
{
	...
	if ( x >= gra.width ) return [0,0,0,0];
	if ( y >= gra.height ) return [0,0,0,0];
...
let g_nest = 0;
//-----------------------------------------------------------------------------
gra.paint2 = function(  x1, y1, col ) 
//-----------------------------------------------------------------------------
{
	g_nest++;

	if ( g_nest > 3000 ) return;
	...
	bc = gra.point( x1, y1 );
	if ( bc[0]+bc[1]+bc[2] > 0 ) return;
	gra.pset( x1, y1 , col );

//gra.circle( x1,y1, 4, [255,255,255,255]);return;

	x = x1+1;y = y1;
	bc = gra.point( x, y );c = bc[0]+bc[1]+bc[2];
	if ( bc == 0 ) gra.paint2( x, y, col );
...
let tblPaint =
[
/*1030*/0x000ff,0,0,0,0,0,
/*1040*/250,30,200,900,32,14,300,6,-1,-1,
/*1050*/0x0000aa,0x000055,0,0,0,0,0,
/*1060*/190,30,280,15,220,95,320,8,370,19,420,40,-1,-1,
/*1070*/0x00aaaa,0x005555,0,0,0,0,0,
/*1080*/348,14,-1,-1,
/*1090*/0xffff00,0,0,3,4,5,
...
/*1340*/639,0,123,175,430,199,999,999,
	...
	gra.paint2( 250,30, [0, 0, 255,255] );
//	gra.paint( 200,900, [0, 0, 255,255] );
//	gra.paint( 32,14, [0, 0, 255,255] );
//	gra.paint( 300,6, [0, 0, 255,255] );
